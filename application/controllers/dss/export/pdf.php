...
class Pdf extends CI_Controller {
    ...
    public function pegawaiPuskesmas() {
        $query['data1'] = $this->m_puskesmas->getListPegawai($this->m_puskesmas->idpuskesmas());
        //load the view and saved it into $html variable
        $html = $this->load->view('export/pegawai', $query, true);
        //this the the PDF filename that user will get to download
        $filename = 'daftar-pegawai-puskesmas-per-' . date("Y-m-d"); //save our workbook as this file name
        $pdfFilePath = $filename . ".pdf";
        //load mPDF library
        $this->load->library('m_pdf');
        //actually, you can pass mPDF parameter on this load() function
        ...
        $pdf->AddPage('L', // L - landscape, P - portrait
                ...
                3); // margin footer
        //generate the PDF!
        $pdf->WriteHTML('<h1>Pegawai Puskesmas ' . $this->m_puskesmas_ . '</h1>');
        $pdf->WriteHTML($html);
        //offer it to user via browser download! (The PDF won't be saved on your server HDD)
        $pdf->Output($pdfFilePath, "D");
    }
    ...
    public function kaderByIdPuskesmas() {
        ...
        $pdf->AddPage('L', // L - landscape, P - portrait
                ...
                3); // margin footer
        //generate the PDF!
        $pdf->WriteHTML('<h1>Kader Puskesmas ' . $this->m_puskesmas_ . '</h1>');
        $pdf->WriteHTML($html);
        //offer it to user via browser download! (The PDF won't be saved on your server HDD)
        $pdf->Output($pdfFilePath, "D");
    }

...
class Delete extends REST_Controller {
    ...
    function pegawai_get($id) {
        $deletePegawai = $this->db->query("delete from pegawai_puskesmas where id='$id'");
        $response = array();
        if ($deletePegawai) {
            $deleteUser = $this->db->query("delete from user where id_pegawai='$id' and role = '%puskesmas");
            if ($deleteUser) {
                $response = array(
                    'status' => true,
    ...
    function posbindu_post() {
        $this->m_puskesmas->deletePosbindu($this->input->post['id']);
        $response = array(
            'status' => true,
            'message' => 'Hapus Posbindu Berhasil'
        );
        $this->response($response, 200);
    }
    ...
    public function pasien($id) {
        $response = array();
        if ($this->m_puskesmas->deletepasien($id)) {
            $response = array(
                'status' => true,
    ...
    public function riwayat($id) {
        $response = array();
        if ($this->m_puskesmas->deleteriwayat($id)) {
            $response = array(
                'status' => true,
